...
Claude scheduled tasks are session-only. If the approved cadence has an idle gap long enough for Claude to retire the coordinator, create a keepalive-only schedule within that same session. A keepalive may call `CronList` to confirm the expected tasks, but it must not read discovery sources, create cards, process jobs, call the status API, or change schedules. Session tasks also expire after seven days, so a recurring discovery prompt renews both tasks once they are five days old and records the new IDs and creation time in `discovery-state.json`.
...
- When Muesli is installed, or Granola and Notion are connected, check each available source during every scheduled discovery pass. For Muesli and Granola, scan every meeting from midnight through now in the user's local timezone, read relevant original transcripts and notes, and create cards only for user-owned decisions or follow-ups not already covered by any card status or active job. During an overlap or migration, treat semantically identical meetings and actions across Muesli and Granola as duplicates. Use a stable source-meeting-plus-action dedupe key. Check new Muesli dictations within the normal incremental window. For Notion, read recent or newly linked meeting notes, sprint plans, project briefs and decisions using a window since the previous discovery pass. Report source failure separately from a successful check with no useful work.
